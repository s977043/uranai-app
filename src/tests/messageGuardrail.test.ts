...
import {
  inspectMessage,
  passesMessageGuardrail,
} from "@/domain/messageGuardrail";

/**
 * docs/concept-board.md 「ブランド人格 > NG表現」を機械的に検証するテスト。
 * この層は既知のNG表現だけを検査し、包括的なSafety判定は行わない。
 */
...
describe("inspectMessage: 主語と述語の距離を変えても検出する", () => {
  ...
  it("読点をまたぐ共起は検出しない（文をまたいだ誤検出を避ける設計）", () => {
    expect(
      passesMessageGuardrail("その症状は、あなたのせいではありません。"),
    ).toBe(true);
  });
});
...
describe("inspectMessage: 表記ゆれ", () => {
  it.each([
    ...
    expect(passesMessageGuardrail(text)).toBe(false);
  });
});
