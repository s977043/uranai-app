...
for (const pilotCase of rehearsal.cases) {
  ...
  const refs = [pilotCase.target_metric_ref, ...(pilotCase.guardrail_metric_refs ?? [])];
  for (const ref of refs) {
    const metric = metricRefs.get(ref);
    assert(metric, `${pilotCase.id}: unknown metric ref ${ref}`);
    assert(metric.status === "active", `${pilotCase.id}: metric ref ${ref} must have active definition`);
  }
